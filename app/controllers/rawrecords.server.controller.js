 ...
 * List of Rawrecords
 */
exports.list = function(req, res) { Rawrecord.find().sort('-created').populate('user', 'displayName').exec(function(err, rawrecords) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 */
exports.rawrecordByID = function(req, res, next, id) { Rawrecord.findById(id).populate('user', 'displayName').exec(function(err, rawrecord) {
